...
notify_mic_user() {
	if [[ "$(get_mic_volume)" == "Muted" ]]; then
		notify-send -e -h string:x-canonical-private-synchronous:volume_notif -u low "Mic-Level: Muted"
	else
		notify-send -e -h int:value:"$(get_mic_volume | sed 's/%//')" -h string:x-canonical-private-synchronous:volume_notif -u low "Mic-Level: $(get_mic_volume)"
	fi
}
